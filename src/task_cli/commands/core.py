...
from ..app import app, service, console, err_console, complete_task_id, resolve_id, get_git_repo_name
...
from ..prompts import RUN_BUG_SYSTEM_PROMPT
from ..formatters import print_tasks, format_date, TICK, CROSS, WARN, BRANCH
...
def list_tasks(
    ...
    if overdue:
        ...
        status = status or TaskStatus.DO
...
def update(
    ...
    new_description: Optional[str] = typer.Argument(None, help="The new description for the task (Optional)"),
    priority: Optional[str] = typer.Option(None, "--priority", "-p", help="Update priority (low, med, high)"),
    project: Optional[str] = typer.Option(None, "--project", help="Change the project name"),
    status: Optional[str] = typer.Option(None, "--status", "-s", help="Change status (do, doing, done)")
):
...
def start(task_ref: str = typer.Argument(..., autocompletion=complete_task_id, help="Task ID or new task description")):
    ...
    if len(matches) == 1:
        ...
        console.print(
            f"[green]▶ Started[/green] task [bold cyan]{task.id}[/bold cyan]  [dim]{task.description}[/dim]"
        )
    elif len(matches) > 1:
        ids = ", ".join(t.id for t in matches)
        err_console.print(f"[red]Error:[/red] Task reference is ambiguous. Matches: {ids}")
        raise typer.Exit(1)
    else:
        task = service.start_time_tracking(description=task_ref)
        console.print(
            f"[green]✔ Created + Started[/green] task [bold cyan]{task.id}[/bold cyan]  [dim]{task.description}[/dim]"
        )
...
def stop(task_id: Optional[str] = typer.Argument(None, autocompletion=complete_task_id, help="Task ID (optional)")):
    ...
    console.print(
        f"[green]■ Stopped[/green] task [bold cyan]{task.id}[/bold cyan]  [dim]{task.description}[/dim]\n"
...
def run(ctx: typer.Context, command: List[str] = typer.Argument(..., help="Terminal command to execute")):
    ...
    sys_prompt = RUN_BUG_SYSTEM_PROMPT
